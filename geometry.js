...
export function projectPointOnLine(p, lS, lE) {
    ...
    if (l2 === 0) return lS; // Fallback for 0 length
...
export function rotateLine(line, pivot, angleDeg) {
...
export function lineCircleIntersection(l, center, radius) {
    const proj = projectPointOnLine(center, l.p1, l.p2);
    ...
    const len = dist(l.p1, l.p2);
    if (len === 0) return [];

    const ux = (l.p2.x - l.p1.x) / len;
    const uy = (l.p2.y - l.p1.y) / len;
...
export function computeFullPath(C_points, HC, A1, B1, HR, fastSweep = false) {
    ...
    const circuitVisited = new Array(C_points.length).fill(false);
    const hitIndices = new Array(C_points.length).fill(-1);
...
export function evaluateGrashof(HC, A1, B1, HR) {
    ...
    let inputType = 'Rocker';
    let outputType = 'Rocker';

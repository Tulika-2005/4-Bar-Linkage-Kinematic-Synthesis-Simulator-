...
export function drawGrid(ctx, width, height, transform) {
    ...
    ctx.strokeStyle = 'rgba(233, 236, 239, 0.3)';
    ctx.lineWidth = 1;
    ctx.setLineDash([]);
    const gridSize = 50;
 ...
 */
export function drawLinkage(ctx, HC, HR, A, B, C_pt, transform, isGhost = false, currentGamma = 90, gammaThreshold = 45, gammaEnabled = true) {
    ...
    if (isGhost) {
        ...
        const isBadGamma = gammaEnabled && (currentGamma < gammaThreshold || currentGamma > (180 - gammaThreshold));
